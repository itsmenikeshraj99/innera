...
type Props = NativeStackScreenProps<RootStackParamList, 'EnglishSubjects'>;

const EDITION_COLORS: Record<string, { color: string; label: string }> = {
  current: { color: '#1A3A5C', label: 'Current' },
  previous: { color: '#6B7A2A', label: 'Previous' },
  elective: { color: '#6C3483', label: 'Elective' },
  novel: { color: '#7B3F00', label: 'Novel' },
};

export default function EnglishSubjectsScreen({ navigation, route }: Props) {
          ...
          renderItem={({ item }) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => navigation.navigate('EnglishChapters', {
                  classId,
                  className,
                  bookCode: item.code,
              ...
              >
                ...
                <View style={styles.textBlock}>
                  <View style={styles.nameRow}>
                    <Text style={[styles.bookName, { color: editionInfo.color }]}>
                      {item.bookName}
                    </Text>
...
const styles = StyleSheet.create({
  ...
  bookName: { fontSize: 15, fontWeight: '700' },
  editionBadge: {
