...
type Props = NativeStackScreenProps<RootStackParamList, 'HindiSubjects'>;

const EDITION_COLORS: Record<string, { color: string; label: string }> = {
  current: { color: '#8B1A1A', label: 'Current' },
  previous: { color: '#6B7A2A', label: 'Previous' },
  elective: { color: '#6C3483', label: 'Elective' },
  both: { color: '#1A5276', label: 'Core+Elective' },
  novel: { color: '#7B3F00', label: 'Novel' },
};

export default function HindiSubjectsScreen({ navigation, route }: Props) {
  ...
  const fetchBooks = async () => {
    try {
      const docSnap = await getDoc(doc(db, 'syllabus', 'hindi', 'classes', classId));
      if (docSnap.exists()) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>हिंदी (Hindi)</Text>
      </View>

      {loading ? (
        <ActivityIndicator size="large" color="#8B1A1A" style={{ marginTop: 40 }} />
      ) : (
        <FlatList
          data={books}
          ...
          renderItem={({ item }) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => navigation.navigate('HindiChapters', {
                  classId,
                  className,
                  bookCode: item.code,
